...
import com.iwdael.pdfviewer.util.PageSizeCalculator;
...
class PdfFile {
    ...
    PdfFile(CoreSource coreSource, FitPolicy pageFitPolicy, Size viewSize, int[] originalUserPages,
...
//            pagesCount = 1;
            ...
            pagesCount = coreSource.getPageCount();
//            pagesCount = 1;
        }

        for (int i = 0; i < pagesCount; i++) {
            Size pageSize = coreSource.getPageSize(documentPage(i));
    ...
    public void renderPageBitmap(Bitmap bitmap, int pageIndex, Rect bounds, boolean annotationRendering) {
        int docPage = documentPage(pageIndex);
        coreSource.renderPageBitmap(bitmap, docPage, -bounds.left, -bounds.top, bounds.width(), bounds.height(), annotationRendering);
    }
    ...
    public List<Link> getPageLinks(int pageIndex) {
        int docPage = documentPage(pageIndex);
        return coreSource.getPageLinks(docPage);
    }

    public RectF mapRectToDevice(int pageIndex, int startX, int startY, int sizeX, int sizeY, RectF rect) {
        int docPage = documentPage(pageIndex);
        return coreSource.mapRectToDevice(docPage, startX, startY, sizeX, sizeY, 0, rect);
    }

    public void dispose() {
        if (coreSource != null) {
            coreSource.closeDocument();
        }
